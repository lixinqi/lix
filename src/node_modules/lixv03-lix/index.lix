...
generateSeq := [expr env ctx def]->{
	funcName := getUniqVarName call
	getCaseNo := counterGenerator call
	chainDefer := [
		'function () {\n'
...
generateIf := [expr env ctx def]->{
	expr.0 map [i]->{
		...
		if (i.1 == 'else') {
			...
			ret = (i generateIf env ctx def)
		} else {
			ret = [ 'if (' (i.0 generate env ctx def) ') {\n'
...
generatePropertyName := [expr env ctx def]->{
	ret := null
	if (expr.1 == '{atomic}') {
		ret = expr generateAtomic env ctx def
	} (expr.1 == '{index}') {
		ret = expr.0 generateAtomic env ctx def
	}
	ret
}
...
getVarName := [expr]->{
	ret := null
	if (expr.1 == '{atomic}') {
		ret = expr generateAtomic {} ctx0
	} (expr.1 == '{.}') {
		ret = expr.0 getVarName
	}
	ret
}
...
generateField := [expr env ctx def]->{
	ret := null
	if (expr.1 == '{atomic}') {
		ret = '.' + (expr generateAtomic env ctx0 def)
	} (expr.1 == '{index}') {
		ret = ['[' (expr.0 generate env ctx def) ']'] join
	}
	ret
}
...
getStartLayout := [makeExpr]->{
	ret := null
	if (makeExpr) {
		ret = exprStartLayout
	} else {
		ret = statementStartLayout
	}
	ret
}

generateStart := [expr env ctx def makeExpr]->{
	layout := getStartLayout call makeExpr
	layout call [return]->{
		def := GenerateDefNew call
...
compile := [expr makeExpr]->{
	getCount = counterGenerator call
	libs := env0 | map [i]->{i} | .Join "\n"
	flattenedExpr := expr | flatten [i]->{} [i]->{i}
	code := flattenedExpr generateStart (env_new call) ctx0 def0 makeExpr
	[libs "\n" code] join
}
